  ...
  return (
    <section className="stack-page">
      ...
      <Card className="notice-card">
        ...
        <div>
          <h2>{t("care.supportTitle")}</h2>
          <p>{t("care.supportText")}</p>
          <PrimaryButton href="/symptom-check">{t("home.start")}</PrimaryButton>
      ...
      <Card className="tool-section">
        <div className="card-title-row">
          <div>
            <p className="eyebrow">{t("care.comparisonEyebrow")}</p>
            <h2>{t("care.comparisonTitle")}</h2>
          </div>
        </div>
      ...
      <div className="care-detail-grid">
        {careOptions.map((option) => (
          <article className="panel detail-card care-compact-card" key={option.id}>
            ...
            <button
              className="care-details-toggle"
              onClick={() => setOpenCard(openCard === option.id ? null : option.id)}
              type="button"
            >
            ...
            {openCard === option.id ? (
              <div className="care-details">
                <dl>
                  ...
                  <dt>{t("care.insuranceQuestions")}</dt>
                  <dd>{option.askKeys.map((key) => <span key={key}>{t(key)}</span>)}</dd>
                  <dt>{t("care.relatedNextStep")}</dt>
                  <dd><a className="inline-link" href={option.nextStepHref}>{t(option.nextStepKey)}</a></dd>
                </dl>
              </div>
            ) : null}
          </article>
